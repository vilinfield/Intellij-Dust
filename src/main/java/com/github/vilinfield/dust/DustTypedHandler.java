...
import com.github.vilinfield.dust.psi.DustElseTag;
...
public class DustTypedHandler extends TypedHandlerDelegate
{
    ...
    @NotNull
    @Override
    public Result charTyped(char c, @NotNull Project project, Editor editor, @NotNull PsiFile file)
    {
        int offset = editor.getCaretModel().getOffset();
    ...
    private String getTagName(PsiElement tag)
    {
        ...
        if (tag != null && tag.getChildren().length > 1)
        {
            ...
            if (children.length > 0)
            {
                ...
                if (tagNameEl != null && tagNameEl.getNode().getElementType() == DustTypes.TAG_NAME)
                {
                    ...
                    if (children.length > 1)
                    {
                        ...
                        if (indexDeref != null && indexDeref.getNode().getElementType() == DustTypes.INDEX_DEREF)
                        {
    ...
    {
        PsiElement elementAtCaret = provider.findElementAt(offset - 1, DustLanguage.class);
        PsiElement elseParent = PsiTreeUtil.findFirstParent(elementAtCaret, true, e -> (e instanceof DustElseTag));
